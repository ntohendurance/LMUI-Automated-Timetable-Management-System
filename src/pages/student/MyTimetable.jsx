...
export default function MyTimetable() {
  ...
  }, [semId, user.departmentId, user.level])
  ...
  return (
    <PageWrapper
      title="My Timetable"
      description={`${user.department || ''} — read-only weekly schedule.`}
      actions={
    ...
    >
      ...
      {loading ? (
        ...
        <div className="flex items-center gap-3 rounded-xl border border-navy-100 bg-white p-6 text-sm text-navy-500 shadow-sm">
          ...
          No classes found for your department and level this semester.
        </div>
      ) : (
        <>
          <TimetableGrid slots={slots} cellFields={['title', 'lecturer', 'room']} />
          <p className="mt-3 text-xs text-navy-400">
            Last updated: {selected?.publishedAt ? formatDate(selected.publishedAt) : '—'}.
          </p>
        </>
      )}
    </PageWrapper>
