...
import { PERIOD_TO_LABEL } from '../../api/adapters.js'
...
export default function StudentCourses() {
  ...
  return (
    <PageWrapper
      title="Courses"
      description={`Courses for your department & level — ${activeSemester?.label || 'this semester'}.`}
    >
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {courses.map((c, i) => (
            <div key={c.id} className="animate-fade-up rounded-2xl border border-navy-100 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-gold-300 hover:shadow-md" style={{ animationDelay: `${i * 60}ms` }}>
              ...
              <div className="mt-4 space-y-2 border-t border-navy-50 pt-4 text-xs text-navy-500">
                <p>
                  <span className="text-navy-400">Lecturer: </span>
                  <span className="font-medium text-navy-700">{c.lecturerName || 'TBA'}</span>
                </p>
                {c.preferredTimeSlot && (
                  <p className="inline-flex items-center gap-1.5">
                    <Clock size={13} className="text-gold-500" />
                    <span className="font-medium text-navy-700">{PERIOD_TO_LABEL[c.preferredTimeSlot] || c.preferredTimeSlot}</span>
                  </p>
                )}
              </div>
            </div>
